...
frappe.query_reports["Employee Monthly Attendance"] = {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if ((column.fieldname === "employee" || column.fieldname === "employee_name") && data && data.employee) {
			value =
				"<a href='#' onclick='return false;' data-employee='" +
				frappe.utils.escape_html(data.employee) +
				"' class='emp-attendance-link'>" +
				value +
				"</a>";
		}
	...
	after_datatable_render: function (datatable_obj) {
		$(datatable_obj.wrapper).on("click", ".emp-attendance-link", function (e) {
			e.preventDefault();
			const employee = $(this).attr("data-employee");
			frappe.route_options = { employee: employee };
			frappe.set_route("query-report", "Employee Checkin Report");
		});
	},
};
