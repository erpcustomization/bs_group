	...
	if filters.summarized_view:
		columns.extend(
			[
				...
				{
					"label": _("Expected Working Day(s)"),
					"fieldname": "expected_working_days",
					"fieldtype": "Float",
					"width": 160,
				},
				{
					"label": _("Expected Payable Day(s)"),
					"fieldname": "expected_payable_days",
					"fieldtype": "Float",
					"width": 160,
				},
				{
					"label": _("Worked Day(s)"),
					"fieldname": "worked_days",
					"fieldtype": "Float",
					"width": 120,
				},
				{
					"label": _("Paid Off Day(s)"),
					"fieldname": "paid_off_days",
					"fieldtype": "Float",
					"width": 130,
				},
				{
					"label": _("Total Payable Days"),
					"fieldname": "total_payable_days",
					"fieldtype": "Float",
					"width": 140,
				},
				{
					"label": _("Absent (LOP)"),
					"fieldname": "absent_lop",
					"fieldtype": "Float",
					"width": 120,
				},
				{
		...
		columns.append(
			{"label": _("Shift"), "fieldname": "shift", "fieldtype": "Data", "width": 120, "hidden": 1}
		)
...
def get_data(filters: Filters, attendance_map: dict, leave_type_map: dict) -> list[dict]:
	...
	if filters.group_by:
		...
		for value in group_by_param_values:
			...
			records = get_rows(employee_details[value], filters, holiday_map, attendance_map, leave_type_map)
		...
		data = get_rows(employee_details, filters, holiday_map, attendance_map, leave_type_map)
...
def get_attendance_map(filters: Filters) -> dict:
	...
	# Shift is not shown as a separate column (hidden), so all shifts for an
	# employee are collapsed into a single row instead of one row per shift.
...
def get_lwp_leave_types() -> set:
	"""Leave Types configured as Leave Without Pay - these should read as
	Absent in the detailed attendance view, not as the leave type name."""
...
def get_rows(
	employee_details: dict, filters: Filters, holiday_map: dict, attendance_map: dict, leave_type_map: dict
) -> list[dict]:
	records = []
	default_holiday_list = frappe.get_cached_value("Company", filters.company, "default_holiday_list")
	...
	for employee, details in employee_details.items():
		...
		if filters.summarized_view:
			...
			payroll_summary = get_payroll_summary(employee, filters, holidays, attendance)
			...
			attendance_for_employee = get_attendance_status_for_detailed_view(
				employee, filters, employee_attendance, holidays, leave_type_map
			)
...
def get_attendance_status_for_detailed_view(
	employee: str, filters: Filters, employee_attendance: dict, holidays: list, leave_type_map: dict
) -> list[dict]:
	...
	total_days = get_dates_in_period(filters)
	attendance_values = []
	lwp_leave_types = get_lwp_leave_types()
	...
	for shift, status_dict in employee_attendance.items():
		...
		for d in total_days:
			...
			# Show the actual leave type (e.g. "Sick Leave") instead of the
			# generic "On Leave" label - Leave Without Pay reads as Absent.
			if status == "On Leave":
				leave_type = leave_type_map.get((employee, d))
				status = "Absent" if leave_type in lwp_leave_types else (leave_type or status)

			abbr = status_map.get(status, status or "")
...
def get_payroll_summary(
	employee: str, filters: Filters, holidays: list, attendance: dict
) -> dict[str, float]:
	"""Returns payroll-related computed columns:
	expected_working_days, expected_payable_days, worked_days, paid_off_days,
	total_payable_days, absent_lop, total
	"""
	total_days = get_dates_in_period(filters)
	total_period_days = len(total_days)

	weekly_offs = sum(1 for d in total_days if get_holiday_status(getdate(d), holidays) == "Weekly Off")
	holidays_count = sum(1 for d in total_days if get_holiday_status(getdate(d), holidays) == "Holiday")

	# Expected Working Day(s): period days excluding weekly offs
	expected_working_days = total_period_days - weekly_offs

	# Expected Payable Day(s): period days excluding weekly offs and holidays
	expected_payable_days = total_period_days - weekly_offs - holidays_count

	# Worked Day(s): Present + WFH (total_present already includes half days)
	worked_days = attendance.get("total_present", 0)

	# Paid Off Day(s): paid leaves (leave types where is_lwp = 0)
	paid_off_days = get_paid_leave_days(employee, filters)

	# Total Payable Days: worked days + paid leaves
	total_payable_days = worked_days + paid_off_days

	# Absent (LOP): absent days + LWP leaves
	absent_lop = attendance.get("total_absent", 0) + get_lwp_leave_days(employee, filters)

	# Total: total payable days (cross-check = expected_payable_days - absent_lop)
	total = total_payable_days

	return {
		"expected_working_days": expected_working_days,
		"expected_payable_days": expected_payable_days,
		"worked_days": worked_days,
		"paid_off_days": paid_off_days,
		"total_payable_days": total_payable_days,
		"absent_lop": absent_lop,
		"total": total,
	}


def get_paid_leave_days(employee: str, filters: Filters) -> float:
	"""Returns total paid leave days (leave types where is_lwp = 0)."""
	Attendance = frappe.qb.DocType("Attendance")
	...
	return result[0].leave_days or 0.0 if result else 0.0


def get_lwp_leave_days(employee: str, filters: Filters) -> float:
	"""Returns total Leave Without Pay days."""
	Attendance = frappe.qb.DocType("Attendance")
	LeaveType = frappe.qb.DocType("Leave Type")
	day_case = frappe.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(1)
	sum_leave_days = Sum(day_case).as_("leave_days")
