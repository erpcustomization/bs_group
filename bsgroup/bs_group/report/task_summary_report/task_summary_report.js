...
frappe.query_reports["Task Summary Report"] = {
    ...
    _dnd_key:       "frappe_report_order_task_summary",
    _dnd_rendering: false,
    _dnd_state:     null,   // active drag state

    // ── Lifecycle ──────────────────────────────────────────────────────────────
    after_datatable_render: function() {
        let R = frappe.query_reports["Task Summary Report"];
        if (R._dnd_rendering) return;

        let data = frappe.query_report.data;
        if (!data || !data.length) return;

        // Apply saved order: sort data in-place then re-render once (guarded).
        ...
        if (saved.length) {
            ...
            // after_datatable_render re-fires inside render_datatable but returns
            // early due to the flag above, so we continue here.
        }
    ...
    // ── Drag binding ──────────────────────────────────────────────────────────
    // Attaches mousedown on the container; mousemove/mouseup go on document.
    _dnd_bind: function() {
        let R  = frappe.query_reports["Task Summary Report"];
        let el = document.querySelector(".datatable .dt-scrollable");
        if (!el) return;

        // Remove previous listeners so re-binding after render doesn't stack.
        ...
        function move(e) {
            ...
            // Slide clone with cursor.
            s.clone.style.top = (s.cloneStartTop + e.clientY - s.startY) + "px";

            // Detect which row the cursor is over.
            ...
            if (overRow && overRow !== s.row) {
                ...
                if (sameGroup) {
                    ...
                    overRow.style[s.insertBefore ? "borderTop" : "borderBottom"] =
                        "2px solid #1565c0";
                    s.targetRow = overRow;
                } else {
                    s.targetRow = null;
        ...
        function up(e) {
            ...
            // Splice data to the new position.
            let data = frappe.query_report.data;
            let [item] = data.splice(fromIdx, 1);
            // Adjust target index after removal, then offset for above/below.
            let adj = toIdx > fromIdx ? toIdx - 1 : toIdx;
            data.splice(s.insertBefore ? adj : adj + 1, 0, item);

            // Persist and immediately re-render — this is what makes it stick.
            R._dnd_save(data.map(r => r.name).filter(Boolean));
            R._dnd_rendering = true;
            frappe.query_report.render_datatable();
            R._dnd_rendering = false;
            // Re-bind drag on the freshly rendered DOM.
            R._dnd_bind();
        }
    ...
    _dnd_save: function(order) {
        try { localStorage.setItem(
            frappe.query_reports["Task Summary Report"]._dnd_key,
...
{ fieldname: "active",      label: __("Active (Exclude Completed)"),fieldtype: "Check" },
        { fieldname: "active_amc",  label: __("Active AMC Only"),          fieldtype: "Check" },
    ...
    formatter: function(value, row, column, data, default_formatter) {
        ...
        const rname = `frappe.query_reports['Task Summary Report']`;
